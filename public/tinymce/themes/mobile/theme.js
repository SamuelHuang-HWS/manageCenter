...
(function (domGlobals) {
    ...
    var getByIndex = function (component, hConfig, hState, index) {
      ...
      return Option.from(items[index]).fold(function () {
        return Result.error('No element found with user ' + index);
    ...
    var sortKeys = function (label, keyName, array, order) {
      try {
        var sorted = sort(array, function (a, b) {
          ...
          if (aIndex === -1) {
            throw new Error('The ordering for ' + label + ' does not have an entrys for ' + aKey + '.\nOrder specified: ' + JSON.stringify(order, null, 2));
          }
          if (bIndex === -1) {
            throw new Error('The ordering for ' + label + ' does not have an entrys for ' + bKey + '.\nOrder specified: ' + JSON.stringify(order, null, 2));
          }
          if (aIndex < bIndex) {
            return -1;
